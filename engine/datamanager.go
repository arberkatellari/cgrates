...
func NewDBConnManager(dataDBs map[string]DataDB, dataDBCfg *config.DataDbCfg) *DBConnManager {
...
func (dbConns *DBConnManager) GetConn(itemID string) (db DataDB, dbConnCfg *config.DBConn, err error) {
	...
	if db, ok = dbConns.dataDBs[dbConnID.DataDBID]; !ok {
		...
		return dbConns.dataDBs[utils.MetaDefault], dbConns.dataDBCfg.DBConns[dbConnID.DataDBID], nil
	}
	dbConnCfg = dbConns.dataDBCfg.DBConns[dbConnID.DataDBID]
	return
}
