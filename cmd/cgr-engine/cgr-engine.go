...
func main() {
	...
	if cfg.ListenCfg().BiJSONListen != utils.EmptyString ||
		...
		go func() {
			...
			if smg.ShouldRun() {
				onConns = append(onConns, <-services.SessionSOnBiJSONConnect)
				onDiss = append(onDiss, <-services.SessionSOnBiJSONDisconnect)
			}
			if tS.ShouldRun() {
				onConns = append(onConns, <-services.ThresholdSOnBiJSONConnect)
				onDiss = append(onDiss, <-services.ThresholdSOnBiJSONDisconnect)
			}
