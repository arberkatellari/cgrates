...
func testApierRpcConn(t *testing.T) {
	...
	for i := 1000000; i <= 5000000; i++ {
		acnt := utils.AttrSetAccount{Tenant: "cgrates.org", Account: "test" + strconv.Itoa(i)}
		...
		if err := rater.Call(context.Background(), utils.APIerSv1SetBalance, attrs, &reply1); err != nil {
			t.Error("Got error on APIerSv1SetBalance: ", err.Error())
		} else if reply1 != utils.OK {
			t.Errorf("Calling APIerSv1SetBalance received: %s", reply1)
		}
	}
}
