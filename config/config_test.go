...
func TestCgrCfgJSONDefaultsDiameterAgentCfg(t *testing.T) {
	...
	if !reflect.DeepEqual(cgrCfg.diameterAgentCfg.DictionaryDefaults, testDA.DictionaryDefaults) {
		t.Errorf("expecting: %+v, received: %+v", cgrCfg.diameterAgentCfg.DictionaryDefaults, testDA.DictionaryDefaults)
	}
...
func TestV1GetConfigDiameterAgent(t *testing.T) {
	...
	expected := map[string]any{
		DA_JSN: map[string]any{
			...
			utils.DictionaryDefaultsCfg: true,
