...
func NewDataManager(dbConns *DBConnManager, cfg *config.CGRConfig, connMgr *ConnManager) *DataManager {
	...
	dbConns.dataDBCfg = cfg.DataDbCfg()
	return &DataManager{
		dbConns: dbConns,
		cfg:     cfg,
		connMgr: connMgr,
...
type DBConnManager struct {
	...
	dataDBCfg    *config.DataDbCfg // Hold the data_db config
}

// GetConnID returns the DataDB and DBConn config where the provided itemID is stored
func (dbConns *DBConnManager) GetConnID(itemID string) (db DataDB, dbConnCfg *config.DBConn, err error) {
	dbConns.RLock()
	defer dbConns.RUnlock()
	var ok bool
	dbConnID, ok := dbConns.dataDBCfg.Items[itemID]
	if !ok {
		return nil, nil, fmt.Errorf("Couldnt find item with ID: ", itemID)
	}
	if db, ok = dbConns.dataDBs[dbConnID.DataDBID]; !ok {
		return nil, nil, fmt.Errorf("Couldnt find DataDB with DataDBID: ", dbConnID.DataDBID)
	}
	if dbConnCfg, ok = dbConns.dataDBCfg.DBConns[dbConnID.DataDBID]; !ok {
		return nil, nil, fmt.Errorf("Couldnt find DataDB config with DataDBID: ", dbConnID.DataDBID)
	}
	return
}
...
func (dbConns *DBConnManager) getDataDBForPrefix(prefix string) (dataDB DataDB, err error) {
	...
	dataDB, _, err = dbConns.GetConnID(itemID)
	return
}
		...
		if mustBeCached {
			...
			if err != nil {
				return utils.NewCGRError(utils.DataManager,
					...
					err.Error(),
					fmt.Sprintf("DataManager error <%s>", err.Error(), prfx))
			}
...
func (dm *DataManager) GetFilter(ctx *context.Context, tenant, id string, cacheRead, cacheWrite bool,
	...
	if strings.HasPrefix(id, utils.Meta) {
		...
		dataDB, dbCfg, err := dm.dbConns.GetConnID(utils.MetaFilters)
		if err != nil {
			return nil, err
		}
...
func (dm *DataManager) GetThreshold(ctx *context.Context, tenant, id string,
	...
	dataDB, dbCfg, err := dm.dbConns.GetConnID(utils.MetaThresholds)
	if err != nil {
		return nil, err
	}
...
func (dm *DataManager) GetThresholdProfile(ctx *context.Context, tenant, id string, cacheRead, cacheWrite bool,
	...
	dataDB, dbCfg, err := dm.dbConns.GetConnID(utils.MetaThresholdProfiles)
	if err != nil {
		return nil, err
	}
...
func (dm *DataManager) GetStatQueue(ctx *context.Context, tenant, id string,
	...
	dataDB, dbCfg, err := dm.dbConns.GetConnID(utils.MetaStatQueues)
	if err != nil {
		return nil, err
	}
...
func (dm *DataManager) GetStatQueueProfile(ctx *context.Context, tenant, id string, cacheRead, cacheWrite bool,
	...
	dataDB, dbCfg, err := dm.dbConns.GetConnID(utils.MetaStatQueueProfiles)
	if err != nil {
		return nil, err
	}
...
func (dm *DataManager) GetTrend(ctx *context.Context, tenant, id string,
	...
	dataDB, dbCfg, err := dm.dbConns.GetConnID(utils.MetaTrends)
	if err != nil {
		return nil, err
	}
...
func (dm *DataManager) GetTrendProfile(ctx *context.Context, tenant, id string, cacheRead, cacheWrite bool,
	...
	dataDB, dbCfg, err := dm.dbConns.GetConnID(utils.MetaTrendProfiles)
	if err != nil {
		return nil, err
	}
...
func (dm *DataManager) GetTrendProfileIDs(ctx *context.Context, tenants []string) (tps map[string][]string, err error) {
	...
	var keys []string
	dataDB, _, err := dm.dbConns.GetConnID(utils.MetaTrendProfiles)
	if err != nil {
		return nil, err
	}
	if len(tenants) == 0 {
...
func (dm *DataManager) GetRankingProfile(ctx *context.Context, tenant, id string, cacheRead, cacheWrite bool, transactionID string) (rgp *utils.RankingProfile, err error) {
	...
	dataDB, dbCfg, err := dm.dbConns.GetConnID(utils.MetaRankingProfiles)
	if err != nil {
		return nil, err
	}
...
func (dm *DataManager) GetRankingProfileIDs(ctx *context.Context, tenants []string) (rns map[string][]string, err error) {
	...
	var keys []string
	dataDB, _, err := dm.dbConns.GetConnID(utils.MetaRankingProfiles)
	if err != nil {
		return nil, err
	}
	if len(tenants) == 0 {
...
func (dm *DataManager) GetRanking(ctx *context.Context, tenant, id string, cacheRead, cacheWrite bool, transactionID string) (rn *utils.Ranking, err error) {
	...
	dataDB, dbCfg, err := dm.dbConns.GetConnID(utils.MetaRankings)
	if err != nil {
		return nil, err
	}
...
func (dm *DataManager) GetResource(ctx *context.Context, tenant, id string, cacheRead, cacheWrite bool,
	...
	dataDB, dbCfg, err := dm.dbConns.GetConnID(utils.MetaResources)
	if err != nil {
		return nil, err
	}
...
func (dm *DataManager) GetResourceProfile(ctx *context.Context, tenant, id string, cacheRead, cacheWrite bool,
	...
	dataDB, dbCfg, err := dm.dbConns.GetConnID(utils.MetaResourceProfiles)
	if err != nil {
		return nil, err
	}
...
func (dm *DataManager) GetIPAllocations(ctx *context.Context, tenant, id string, cacheRead, cacheWrite bool,
	...
	dataDB, dbCfg, err := dm.dbConns.GetConnID(utils.MetaIPAllocations)
	if err != nil {
		return nil, err
	}
...
func (dm *DataManager) GetIPProfile(ctx *context.Context, tenant, id string, cacheRead, cacheWrite bool,
	...
	dataDB, dbCfg, err := dm.dbConns.GetConnID(utils.MetaIPProfiles)
	if err != nil {
		return nil, err
	}
...
func (dm *DataManager) GetRouteProfile(ctx *context.Context, tenant, id string, cacheRead, cacheWrite bool,
	...
	dataDB, dbCfg, err := dm.dbConns.GetConnID(utils.MetaRouteProfiles)
	if err != nil {
		return nil, err
	}
...
func (dm *DataManager) GetAttributeProfile(ctx *context.Context, tenant, id string, cacheRead, cacheWrite bool,
	...
	if strings.HasPrefix(id, utils.Meta) {
		...
		dataDB, dbCfg, err := dm.dbConns.GetConnID(utils.MetaAttributeProfiles)
		if err != nil {
			return nil, err
		}
...
func (dm *DataManager) GetChargerProfile(ctx *context.Context, tenant, id string, cacheRead, cacheWrite bool,
	...
	dataDB, dbCfg, err := dm.dbConns.GetConnID(utils.MetaChargerProfiles)
	if err != nil {
		return nil, err
	}
...
func (dm *DataManager) GetItemLoadIDs(ctx *context.Context, itemIDPrefix string, cacheWrite bool) (loadIDs map[string]int64, err error) {
	...
	dataDB, dbCfg, err := dm.dbConns.GetConnID(utils.MetaLoadIDs)
	if err != nil {
		return nil, err
	}
...
func (dm *DataManager) GetRateProfile(ctx *context.Context, tenant, id string, cacheRead, cacheWrite bool,
	...
	dataDB, dbCfg, err := dm.dbConns.GetConnID(utils.MetaRateProfiles)
	if err != nil {
		return nil, err
	}
...
func (dm *DataManager) GetRateProfileRates(ctx *context.Context, args *utils.ArgsSubItemIDs, needIDs bool) (rateIDs []string, rates []*utils.Rate, err error) {
	...
	dataDB, _, err := dm.dbConns.GetConnID(utils.MetaRateProfiles)
	if err != nil {
		return
	}
...
func (dm *DataManager) GetActionProfile(ctx *context.Context, tenant, id string, cacheRead, cacheWrite bool,
	...
	dataDB, dbCfg, err := dm.dbConns.GetConnID(utils.MetaActionProfiles)
	if err != nil {
		return nil, err
	}
...
func (dm *DataManager) ReconnectAll(d map[string]DataDBDriver) {
	...
	for dataDBKey := range dm.dbConns.dataDBs {
...
func (dm *DataManager) GetIndexes(ctx *context.Context, idxItmType, tntCtx, idxKey, transactionID string,
	...
	dataDB, dbCfg, err := dm.dbConns.GetConnID(idxItmType)
	if err != nil {
		return nil, err
	}
...
func (dm *DataManager) GetAccount(ctx *context.Context, tenant, id string) (ap *utils.Account, err error) {
	...
	dataDB, dbCfg, err := dm.dbConns.GetConnID(utils.MetaAccounts)
	if err != nil {
		return nil, err
	}
