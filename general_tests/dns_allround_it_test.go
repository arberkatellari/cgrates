...
func TestDNSAllRound1(t *testing.T) {
	...
	testEnv := TestEnvironment{
		Name:       "TestDNSAllRound1",
		ConfigPath: path.Join(*dataDir, "conf", "samples", cfgDir),
		TpPath:     path.Join(*dataDir, "tariffplans", "dnsagent"),
	}
	client, dnsCfg, shutdown, err := testEnv.Setup(t, *waitRater)
	if err != nil {
		t.Fatal(err)
	}
	defer shutdown()
	...
	t.Run("ReloadConfigOK", func(t *testing.T) {
		...
		if err := client.Call(context.Background(), utils.ConfigSv1ReloadConfig, &config.ReloadArgs{
			Tenant:  "cgrates.org",
			Path:    path.Join(*dataDir, "conf", "samples", cfgDir),
...
func TestDNSAllRound2(t *testing.T) {
	...
	testEnv := TestEnvironment{
		Name:       "TestDNSAllRound2",
		ConfigPath: path.Join(*dataDir, "conf", "samples", "dnsagent_reload"),
		TpPath:     path.Join(*dataDir, "tariffplans", "dnsagent"),
	}
