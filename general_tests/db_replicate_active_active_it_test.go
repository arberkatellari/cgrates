...
"admins": {
	...
	t.Run("Engine1SetData", func(t *testing.T) {
		...
		t.Run("GetScheduledActions", func(t *testing.T) {
			// engine 2 shouldnt have schdeduled actions since filtered by nodeID
			if err := client1.Call(context.Background(), utils.ActionSv1GetScheduledActions, &utils.CGREvent{}, new(string)); err != nil {
				t.Error("Unexpected error: ", err)
			}
			if err := client2.Call(context.Background(), utils.ActionSv1GetScheduledActions, &utils.CGREvent{}, new(string)); err != nil {
				t.Error("Unexpected error: ", err)
			}
		})
	})
